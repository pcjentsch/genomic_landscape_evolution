...
function make_antigenic_map()
    ...
    for dataset in datasets[2:2]
        ...
        @info "Plotting..."
        @showprogress for (key, gdf) in pairs(groupby(unique_df, :week_submitted))
            o = ash(gdf.mds_x, gdf.mds_y; rngx=x_grid, rngy=y_grid, mx=300, my=300)
            p = plot()
            scatter!(p, gdf.mds_x, gdf.mds_y; xlims=bounds_x, ylims=bounds_y, markersize=1.5,
                markerstrokewidth=0.3,
                size=(400, 300),
                plotting_settings...)
            plot!(p, o.rngx, o.rngy, o.z; title=key.week_submitted, xlims=bounds_x, ylims=bounds_y, plotting_settings...)
